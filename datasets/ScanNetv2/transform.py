...
class MixupScene(object):
    def __call__(self, data_dict_1, data_dict_2):
        assert "coord" in data_dict_1.keys() and "coord" in data_dict_2.keys()
        if "coord" in data_dict_1.keys():
            data_dict_1["coord"] = np.concatenate((data_dict_1["coord"], data_dict_2["coord"]), axis=0)
        if "color" in data_dict_1.keys():
            data_dict_1["color"] = np.concatenate((data_dict_1["color"], data_dict_2["color"]), axis=0)
        if "normal" in data_dict_1.keys():
            data_dict_1["normal"] = np.concatenate((data_dict_1["normal"], data_dict_2["normal"]), axis=0)
        if "segment" in data_dict_1.keys():
            data_dict_1["segment"] = np.concatenate((data_dict_1["segment"], data_dict_2["segment"]), axis=0)
        if "instance" in data_dict_1.keys():
            max_ins_id_1 = np.max(data_dict_1["instance"])
            data_dict_1["instance"] = np.concatenate((data_dict_1["instance"], data_dict_2["instance"] + max_ins_id_1 + 1), axis=0)
        return data_dict_1
