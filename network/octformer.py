...
class BFANet_SegHeader(torch.nn.Module):
    ...
    def __init__(
        ...
        self.classifier_feat = torch.nn.Sequential(
            torch.nn.Linear(fpn_channel, fpn_channel),
            torch.nn.BatchNorm1d(fpn_channel),
            torch.nn.LeakyReLU(),
            torch.nn.Linear(fpn_channel, 128),
            torch.nn.BatchNorm1d(128),
            torch.nn.LeakyReLU()
        )

        self.margin_feat = torch.nn.Sequential(
            torch.nn.Linear(fpn_channel, fpn_channel),
            torch.nn.BatchNorm1d(fpn_channel),
            torch.nn.LeakyReLU(),
            torch.nn.Linear(fpn_channel, 128),
        ...
        self.classifier = torch.nn.Sequential(
            ...
            torch.nn.Linear(64, out_channels, bias=True))

        self.margin_score = torch.nn.Sequential(
            torch.nn.Dropout(dropout[0]),
            torch.nn.Linear(128, 64),
            torch.nn.BatchNorm1d(64),
            torch.nn.PReLU(),
        ...
        self.classifier_v2 = torch.nn.Sequential(
            torch.nn.Dropout(dropout[0]),
            torch.nn.Linear(128, 64),
            torch.nn.BatchNorm1d(64),
            torch.nn.PReLU(),
        ...
        self.margin_score_v2 = torch.nn.Sequential(
            torch.nn.Dropout(dropout[0]),
            torch.nn.Linear(128, 64),
            torch.nn.BatchNorm1d(64),
            torch.nn.PReLU(),
        ...
        # # ####ES Attention
        self.scale = 1.0
        self.sem_qkv = torch.nn.Sequential(
            torch.nn.Linear(128, 128 * 3, bias=True),
            torch.nn.BatchNorm1d(128 * 3),
            torch.nn.LeakyReLU()
        )
        self.margin_qkv = torch.nn.Sequential(
            torch.nn.Linear(128, 128 * 3, bias=True),
            torch.nn.BatchNorm1d(128 * 3),
            torch.nn.LeakyReLU()
        )
        ...
        self.fusion_q = torch.nn.Sequential(
    ...
    def forward(self, features: Dict[int, torch.Tensor], octree: Octree,
        ...
        for i in range(self.num_up):
            out = self.up_conv[i](out, octree, depth_max + i)  # up sample
        ...
        # # # qkv
        K, H, C = 1, 8, 128
        qkv_s = self.sem_qkv(sem_out).reshape(-1, K, 3, H, C // H).permute(2, 0, 3, 1, 4)
        qkv_m = self.margin_qkv(margin_out).reshape(-1, K, 3, H, C // H).permute(2, 0, 3, 1, 4)
        q_s, k_s, v_s = qkv_s[0], qkv_s[1], qkv_s[2]  # (N, H, K, C')
        q_m, k_m, v_m = qkv_m[0], qkv_m[1], qkv_m[2]  # (N, H, K, C')
        cat_q = torch.cat((q_s, q_m), dim=-1).reshape(-1, C * 2)
        q_all = self.fusion_q(cat_q)
        q_all = q_all.reshape(-1, H, K, C // H)
        #
        # # attn
        attn = q_all @ k_s.transpose(-2, -1)  # (N, H, K, K)
        attn = self.softmax(attn)
        attn = self.attn_drop(attn)
        sem_out = (attn @ v_s).transpose(1, 2).reshape(-1, C)

        attn = q_all @ k_m.transpose(-2, -1)  # (N, H, K, K)
        attn = self.softmax(attn)
        attn = self.attn_drop(attn)
        margin_out = (attn @ v_m).transpose(1, 2).reshape(-1, C)
